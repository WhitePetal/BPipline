...
public class BRDF_NPR_LUT_Inspector : MShaderGUI
{
	...
	public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
	{
		...
		materialEditor.TexturePropertySingleLine(MakeLable("阴影渐变图"), FindProperty("_RampTex"));
		materialEditor.TexturePropertySingleLine(MakeLable("Albedo"), albedo, FindProperty("_DiffuseColor"));
		materialEditor.ShaderProperty(FindProperty("_SpecularColor"), MakeLable("高光颜色"));
		...
		GUILayout.Space(20);
		materialEditor.ShaderProperty(FindProperty("_Fresnel", properties), MakeLable("Fresnel0"));
		...
		if(level == Level.High)
        {
			...
			materialEditor.TexturePropertySingleLine(MakeLable("DetilNormal"), FindProperty("_DetilNormalTex", properties));
		...
		if(level == Level.High)
        {
			...
			if (pointLightOn)
			{
				materialEditor.ShaderProperty(FindProperty("_PointLightColor", properties), MakeLable("Point Light Color"));
				materialEditor.ShaderProperty(FindProperty("_PointLightPos", properties), MakeLable("Point Light Position"));
		...
		if(level != Level.VeryLow)
        {
			...
			{
				GUILayout.Space(20);
				GUILayout.Label(new GUIContent("闪点"));
				EditorGUI.indentLevel += 2;
		...
		EditorGUI.indentLevel += 2;
		materialEditor.ShaderProperty(FindProperty("_BloomColor"), MakeLable("BloomColor"));
