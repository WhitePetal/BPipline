...
Shader "Lit/PBR/High/PBR_BRDF_LUT_Clip"
{
    Properties
    {
        ...
        _EmissionGloss("Emission Point Gloss", Vector) = (30, 2, 0, 0)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            FragOutput frag (v2f i)
            {
                ...
                shadingParams.fresnelCol = lerp(GET_PROP(_Fresnel), 0.09, oneMinusMetallic);
                ...
                // #ifndef _EmissionPointOff
                // fixed emissionPointMask = SAMPLE_TEXTURE2D(_EmissionPointMask, sampler_EmissionPointMask, i.uv).r;
                // half emissionPoint = EmissionPoint(i.uv, _EmissionPointDensity, _EmissionPointCutoff_EmissionPointFrequency.x, emissionPointMask, _EmissionPointNoiseOffset, _EmissionPointCutoff_EmissionPointFrequency.y,
                // _EmissionPointPulse.x, _EmissionPointPulse.y);
                // half gloss = pow(ndoth, _EmissionGloss.x);
                // pointEmission =  max(0.0, emissionPoint * (gloss + _EmissionGloss.y) * _EmissionPointColor);
                // #endif
        ...
        Pass
        {
            ...
            {
                UNITY_SETUP_INSTANCE_ID(i);
                clip(SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, i.uv.xy).a - 0.5);
                return 0.0;
            }
            ENDHLSL
        }
