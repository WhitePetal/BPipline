Shader "Lit/PBR/High/PBR_BSDF_Alpha_LUT"
{
 Properties
    {
        ...
        _ShiftStrength("高光扰动强度", Range(-2.0, 2.0)) = 0.5
        _AnisoDirectionTex("高光扰动图", 2D) = "white" {}
        [VectorRange(0, 10.0, 1, 10.0, 0, 10.0, 1, 10.0, 0, 1.0, 0, 0.001, 0, 1.0, 0, 0.001)] _Shifts_SpecularWidths("1号高光偏移_2号高光偏移_1号高光末端宽窄_2号高光末端宽窄", Vector) = (0.1, 0.1, -0.5, -0.5)
        [VectorRange(0.01, 1, 0.01, 1, 0, 8, 0, 8)]_Exponents_SpecStrengths("1号高光宽窄_2号高光宽窄_1号高光强度_2号高光强度", Vector) = (0.1, 0.1, 1.0, 1.0)
        _SpecColor1("1号高光颜色", Color) = (1.0, 1.0, 1.0, 1.0)
        _SpecColor2("2号高光颜色", Color) = (1.0, 1.0, 1.0, 1.0)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            #pragma multi_compile_local _EmissionPointOff _EmissionPointLow _EmissionPointMid
            ...
            TEXTURE2D(_AnisoDirectionTex);
            SAMPLER(sampler_AnisoDirectionTex);
            ...
            UNITY_INSTANCING_BUFFER_START(UnityPerMaterial)
                ...
                UNITY_DEFINE_INSTANCED_PROP(float4, _AnisoDirectionTex_ST)
                UNITY_DEFINE_INSTANCED_PROP(half, _ShiftStrength)
                UNITY_DEFINE_INSTANCED_PROP(half4, _Shifts_SpecularWidths)
                UNITY_DEFINE_INSTANCED_PROP(half4, _Exponents_SpecStrengths)
                UNITY_DEFINE_INSTANCED_PROP(half3, _SpecColor1)
                UNITY_DEFINE_INSTANCED_PROP(half3, _SpecColor2)
            ...
            v2f vert (a2v v)
            {
                ...
                o.uv_shift = v.texcoord * GET_PROP(_AnisoDirectionTex_ST).xy + GET_PROP(_AnisoDirectionTex_ST).zw;
            ...
            FragOutput frag (v2f i)
            {
                ...
                shadingParams.albedo = lerp(GET_PROP(_DiffuseColor) * mainTex.rgb, 
                ...
                half shift = SAMPLE_TEXTURE2D(_AnisoDirectionTex, sampler_AnisoDirectionTex, i.uv_shift).r * GET_PROP(_ShiftStrength);
                ...
                for(int lightIndex = 0; lightIndex < _DirectionalLightCount; lightIndex++)
                {
                    ...
                    brdfCol += BSDF_FromLUT_DirLight(shadingParams, shift, i.binormal_world, GET_PROP(_Shifts_SpecularWidths), GET_PROP(_Exponents_SpecStrengths), GET_PROP(_SpecColor1), GET_PROP(_SpecColor2));
                }
                ...
                #ifndef _EmissionPointOff
                half emissionPointMask = SAMPLE_TEXTURE2D(_EmissionPointMask, sampler_EmissionPointMask, i.uv).r;
                half emissionPoint = EmissionPoint(i.uv, GET_PROP(_EmissionPointDensity), GET_PROP(_EmissionPointCutoff_EmissionPointFrequency).x, emissionPointMask, GET_PROP(_EmissionPointNoiseOffset), GET_PROP(_EmissionPointCutoff_EmissionPointFrequency).y,
                ...
                FragOutput output;
                output.color = half4(col, mainTex.a * GET_PROP(_DiffuseColor).a);
        ...
        Pass
        {
            ...
            {
                UNITY_SETUP_INSTANCE_ID(i);
                half dither = InterleavedGradientNoise(i.vertex.xy, 0) * 0.5;
                half alpha = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, i.uv).a * GET_PROP(_DiffuseColor).a;
                clip(alpha + dither - 0.5);
                return 0.0;
            }
            ENDHLSL
        }
        ...
        Pass
        {
            ...
            half4 frag (v2f i) : SV_Target
            {
                ...
                meta.a = mainTex.a * GET_PROP(_DiffuseColor).a;
                return meta;
            }
            ENDHLSL
        }
