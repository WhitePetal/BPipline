...
{
    half3 f = params.fresnelCol + (1.0 - params.fresnelCol) * SAMPLE_TEXTURE2D_LOD(_LUT, sampler_LUT, half2(params.ndotv, 1.0), 0).r;
    return (params.albedo * params.gi.diffuse + params.specular * f * params.gi.specular) * ao;
}
#endif

#if _BRDF_LUT
half3 BRDF_FromLUT_DirLight(ShadingParams params)
{
    ...
    half d = 1.0 / SAMPLE_TEXTURE2D_LOD(_LUT, sampler_LUT, half2(params.roughness, ndoth), 0).b - 1.0;
    ...
    return ((1.0 - f) * params.albedo * ndotl + params.specular * f * g * d / params.ndotv) * _DirectionalLightColors[params.lightIndex].rgb * shadowAtten;
}
#endif

#if _BRDF_LUT || _BSDF_LUT || _BSSSDF_LUT
half3 BRDF_FromLUT_OtherLight(ShadingParams params)
{
    ...
    return ((1.0 - f) * params.albedo * ndotl + params.specular * f * g * d / params.ndotv) * _OtherLightColors[params.lightIndex].rgb * atten /** * shadowAtten **/;
}
#endif
